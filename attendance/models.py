...
class AttendaceRecord(models.Model):
    ...
    def total_attendance_duration_this_month(cls, user):
        if jdatetime.datetime.now().day < 21:
            ...
            j_end_date = jdatetime.date(year, month, 20).replace(month=month+1) if month < 12 else jdatetime.date(year, month, 20).replace(month=1) 
    ...
    def duration(self):
        ...
        if check_in_datetime.time() > datetime.strptime('08:30', '%H:%M').time() and check_in_datetime.time() < datetime.strptime('11:30', '%H:%M').time() and check_out_datetime.time() > datetime.strptime('12:30', '%H:%M').time(): 
            if self.user.sex == 'man':
                duration = check_out_datetime - check_in_datetime - timedelta(hours=1)
            else:
                duration = check_out_datetime - check_in_datetime - timedelta(minutes=30)
        else:
